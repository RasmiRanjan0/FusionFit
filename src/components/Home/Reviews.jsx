...
const Reviews = () => {
  return (
    <section className="text-white px-6 py-20 md:px-16 lg:px-0 max-w-[1440px] mx-auto">
      ...
      {/* Mobile Slider */}
      <Motion.div
        className="flex sm:hidden gap-4 overflow-x-auto -mx-4 px-4 pb-4 scroll-smooth snap-x snap-mandatory hide-scrollbar"
        variants={container}
        initial="hidden"
        whileInView="show"
        viewport={{ once: true }}
      >
        {reviews.map((review, index) => (
          <Motion.div
            key={index}
            variants={fadeUpCard}
            className="min-w-[90%] snap-center bg-[#1a1a1a] rounded-xl p-6 border border-white/5 transition-transform duration-300 ease-in-out hover:border-white/10"
